...
if indicator_ref == 1
    fid3 = fopen('bias_variance', 'w');
else
    fid3 = fopen('variance', 'w');
end
if gamma_choice == 1 && indicator_ref == 1
    fid4 = fopen('PPV_SEN','w');
end

for ind_seq=1:num_seq
    fid1 = fopen(['centroid_',num2str(ind_seq)], 'w');
    fid2 = fopen(['en_centroid_',num2str(ind_seq)], 'w');
    ...
    %3)CH-index to determine number of 20
    %dstnme=sprintf('stu_%d.mat',ind_seq);  
    %eval(['load ',dstnme]);
    best=-1;
    % for i=2:min(20, size(dis2, 1))
    for i=2:min(10, size(dis2, 1))
%	tmp_cl= cluster(Z,'maxclust',i); 
        disp(['seq = ' num2str(ind_seq) ' maxclusters = ', num2str(i)]);
        fflush(stdout);
        [c, pointsInCluster, tmp_cl] = myKmeans(dis2, i);  % for Octave
        enen=zeros(1,N^2);
        pp=cell(1,i);
        W=0;
        for ii=1:i
        ...
        centroid=enen/len;
        [str,pair]=find_cen(centroid,N,1);
        tmp=find_same([num2cell(pair'),pp],2);
        B=sum(tmp.^2);
        if W > 0
    ...
    for i=1:cl_no
	  ...
	  continue;
        ...
        enen=enen+centroid;
        centroid=centroid/len;
        fprintf(fid1, '%s\n', ['>>centroid ',num2str(i),' :(',num2str(len),')']);
        if gamma_choice == 1
    ...
    len=data;
    centroid=enen/len;
    fprintf(fid2, '%s\n',['>>centroid ',num2str(1),' :(',num2str(len),')']);
    if gamma_choice == 1
    ...
    if indicator_ref == 1
        ...
        %bias-variance
        fprintf(fid3, '%s\n', ['>> sequence ', num2str(ind_seq), ': ']);
        fprintf(fid3, '%s\n',['Bias: ',num2str(bias)],'delimiter');
        fprintf(fid3, '%s\n', ['Variance: ',num2str(variance)]);
    else
        [str,pair] = find_cen(centroid,N,1);
        var_dis = find_same({pair',stru{burn_in+1:end}},2);
        var_sq = var_dis.^2;
...
if indicator_ref == 1 && gamma_choice == 1
    ...
    gammalist = [2.^[-5:1:2],6,2.^[3:1:10]];
    for gammaind = 1:16
      fprintf(fid4, '%s\n', ['gamma=',num2str(gammalist(1,gammaind)),': PPV=',num2str(ppv(1,gammaind)),', SEN=',num2str(sen(1,gammaind))]);
    end
end
fclose(fid1);
fclose(fid2);
